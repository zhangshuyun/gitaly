...
func TestListUntrackedRepositories_Exec(t *testing.T) {
	...
	repo1, _, cleaup := gittest.InitBareRepoAt(t, g1Cfg, g1Cfg.Storages[0])
	t.Cleanup(cleaup)
	repo2, _, cleaup := gittest.InitBareRepoAt(t, g1Cfg, g1Cfg.Storages[0])
	t.Cleanup(cleaup)
	repo3, _, cleaup := gittest.InitBareRepoAt(t, g2Cfg, g2Cfg.Storages[0])
	t.Cleanup(cleaup)
