...
func TestSendFrame(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			err := sendFrame(client, []byte(tc.in), time.Now().Add(time.Second))
			if tc.err != nil {
...
func TestSendFrame_timeout(t *testing.T) {
	...
	err := sendFrame(client, frame, time.Now().Add(10*time.Millisecond))
...
func TestRecvFrame(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := recvFrame(client, time.Now().Add(time.Second))
			if tc.err != nil {
				require.Equal(t, tc.err, err)
				return
			}
...
func TestRecvFrame_timeout(t *testing.T) {
	client, _ := socketPair(t)
	_, err := recvFrame(client, time.Now().Add(10*time.Millisecond))
	require.ErrorIs(t, err, os.ErrDeadlineExceeded)
}
