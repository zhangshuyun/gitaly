...
func createNewServer(rubyServer *rubyserver.Server, hookManager hook.Manager, cfg config.Cfg, secure bool, conns *client.Pool) *grpc.Server {
	...
	opts := []grpc.ServerOption{
		grpc.StreamInterceptor(grpc_middleware.ChainStreamServer(
			...
			cache.StreamInvalidator(diskcache.NewLeaseKeyer(), protoregistry.GitalyProtoPreregistered),
		...
		grpc.UnaryInterceptor(grpc_middleware.ChainUnaryServer(
			...
			cache.UnaryInvalidator(diskcache.NewLeaseKeyer(), protoregistry.GitalyProtoPreregistered),
			// Panic handler should remain last so that application panics will be
			// converted to errors and logged
