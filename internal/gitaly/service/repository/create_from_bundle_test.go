...
func TestServer_CreateRepositoryFromBundle_transactional(t *testing.T) {
	...
	// This accounts for the first vote via git-clone(1). Given that git-clone(1) creates all
	// references in a single reference transaction, the hook is once executed for all fetched
	// references (in "prepare" phase, only).
	...
	// which by default creates one reference transaction per reference. We thus see one commit
	// (again, in "prepare" phase only) per reference.
	votingInput = append(votingInput,
