...
func TestFailedReceivePackRequestDueToValidationError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			Desc: "Empty GlId",
			Req:  &gitalypb.SSHReceivePackRequest{Repository: &gitalypb.Repository{StorageName: cfg.Storages[0].Name, RelativePath: repos[0].GetRelativePath()}, GlId: ""},
			Code: codes.InvalidArgument,
		},
		{
			Desc: "Data exists on first request",
			Req:  &gitalypb.SSHReceivePackRequest{Repository: &gitalypb.Repository{StorageName: cfg.Storages[0].Name, RelativePath: repos[0].GetRelativePath()}, GlId: "user-123", Stdin: []byte("Fail")},
			Code: codes.InvalidArgument,
		},
	}
...
func TestReceivePackPushSuccess(t *testing.T) {
	...
	lHead, rHead, err := testCloneAndPush(t, cfg.Storages[0].Path, serverSocketPath, repos[0], pushParams{
...
func TestReceivePackPushFailure(t *testing.T) {
	...
	_, _, err := testCloneAndPush(t, cfg.Storages[0].Path, serverSocketPath, repos[0], pushParams{storageName: "foobar", glID: "1"})
	require.Error(t, err, "local and remote head equal. push did not fail")

	_, _, err = testCloneAndPush(t, cfg.Storages[0].Path, serverSocketPath, repos[0], pushParams{storageName: cfg.Storages[0].Name, glID: ""})
	require.Error(t, err, "local and remote head equal. push did not fail")
}
...
func TestReceivePackPushHookFailure(t *testing.T) {
	...
	_, _, err := testCloneAndPush(t, cfg.Storages[0].Path, serverSocketPath, repos[0], pushParams{storageName: cfg.Storages[0].Name, glID: "1"})
	require.Error(t, err)
...
func TestSSHReceivePackToHooks(t *testing.T) {
	...
	cloneDetails, cleanup := setupSSHClone(t, cfg.Storages[0].Path, repos[0])
	defer cleanup()
...
func testCloneAndPush(t *testing.T, storagePath, serverSocketPath string, testRepo *gitalypb.Repository, params pushParams) (string, string, error) {
