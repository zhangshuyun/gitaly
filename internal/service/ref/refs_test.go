...
func TestSuccessfulFindAllTagsRequest(t *testing.T) {
	...
	// a tag with a large message that cuts off the PGP block
	bigMsgPGPTruncated, err := ioutil.ReadFile("testdata/big_tag_msg.txt")
	require.NoError(t, err)
	bigMsgPGPTruncatedTagID := testhelper.CreateTag(t,
		testRepoCopyPath,
		"v1.1.2",
		commitID,
		&testhelper.CreateTagOpts{
			Message: string(bigMsgPGPTruncated),
		},
	)
	bigMsgPGPTruncated = bytes.TrimRight(bigMsgPGPTruncated, "\n") // newline will be absent
	...
	expectedTags := []*gitalypb.Tag{
		...
		{
			Name:         []byte("v1.1.2"),
			Id:           bigMsgPGPTruncatedTagID,
			TargetCommit: gitCommit,
			Message:      bigMsgPGPTruncated,
			MessageSize:  int64(len(bigMsgPGPTruncated)),
			Tagger: &gitalypb.CommitAuthor{
				Name:     []byte("Scrooge McDuck"),
				Email:    []byte("scrooge@mcduck.com"),
				Date:     &timestamp.Timestamp{Seconds: 1572776879},
